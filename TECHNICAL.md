...
Build 0491 was reported working by the project owner on 2026-09-02. This is a user report, not an independently captured 0491 readback. Its updater is byte-identical after decompression to the captured 0493 updater, and application bytes `[0xD7D6, 0xD8B0)` containing the Product getter/setter implementation are identical. Offline tests combine the exact official 0491 application record with a captured 0493 ROM **in memory only**: this synthetic fixture is not a real 0491 backup. Tests cover model conversion, exact reverse conversion, unchanged code, corruption rejection, and manifest build identification. Both supported builds use the same general restore-risk warning. Firmware version and existing bugs are preserved.
